...
export default class MyGenerator extends CodeGenerator {
  ...
  getInterfaceContent(inter: Interface) {
    ...
    const requestParamss = inter.getRequestParams();
    const paramsCodes = inter.getParamsCode('Params');

    const paramsList = inter.getParamList()
    const getRequestContent = inter.getRequestContent()
    const getBodyParamsCode = inter.getBodyParamsCode()
    ...
    let requestParams = ${JSON.stringify(requestParamss)}
    let paramsCode = ${JSON.stringify(paramsCodes)}
    let paramsList = ${JSON.stringify(paramsList)}
    let getRequestContent = ${JSON.stringify(getRequestContent)}
    let getBodyParamsCode = ${JSON.stringify(getBodyParamsCode)}
    let inter = ${JSON.stringify(inter)}
